...
class User(db.Model, UserMixin):
    ...
    # back_populates = the "author" property of BlogPost
...
class BlogPost(db.Model):
    ...
    # back_populates = the "posts" property/attribute of User
...
def admin_only(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if current_user and getattr(current_user, 'id', None) != 1:
...
def register():
    ...
    if form.validate_on_submit():
        ...
        if db.session.query(User).where(User.email == email).first():
            flash("An user with that email is already registered", "error")
